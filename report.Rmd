...
```{r profit-over-time, out.height='40%'}
plot_df <- df %>% 
...
```{r profit-over-time-smooth, out.height='40%'}
ggplot(plot_df, aes(x = week, y = pat)) + 
...
profit_df <- df %>% 
  filter(!is.na(date)) %>% 
  mutate(profit_gbp = str_replace_all(profit_2, "£", ""), 
         profit_eur = str_replace_all(profit, "€", ""))
profit_eur <- sum(as.numeric(profit_df$profit_eur)) + 
  sum(as.numeric(profit_df$profit_gbp)) * exrate_gbp_eur
profit_eur %<>% round()
total_fee_eur <- 2 * 600 * exrate_gbp_eur
total_fee_eur %<>% round()
...
winnings_sum <- df$profit_pts %>% {sum(.[. > 0])}
losses_sum <- df$profit_pts %>% {-sum(.[. < 0])}
...
Here's a count of the number of winners, losers, places and non-runners we've bet on.
