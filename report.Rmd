...
pacman::p_load(googlesheets, MASS, tidyverse, janitor, filesstrings,
               quantmod, lubridate, magrittr, clipr, knitr, see)
...
while (bad) {
  ...
  if (!"Loading" %in% df$profit_pts) bad <- FALSE
  if (bad) {
    count <- count + 1
    if (count > 99) stop("Tried and failed to read sheet 100 times.")
  }
}
df_deduped <- df %>% 
...
```{r prepare-unmatched}
df_deduped_winonly <- filter(df_deduped, !ew)
